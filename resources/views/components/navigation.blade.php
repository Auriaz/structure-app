    ...
    <nav class="bg-white border-b border-gray-100">
        ...
        @if (Route::has('login'))
            <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
                @auth
                    <x-responsive-nav-link href="{{ url('/dashboard') }}" class="text-sm text-gray-700 dark:text-gray-500 underline">Dashboard</x-responsive-nav-link>
                @else
                    <x-responsive-nav-link href="{{ route('login') }}" class="text-sm text-gray-700 dark:text-gray-500 underline">Log in</x-responsive-nav-link>
